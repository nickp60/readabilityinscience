...
def analyze(path,spath,textType='abstracttext',columnList={'doi','pmid','pubdate_year<>year'},tagger=None,doPreprocessing=1,ignoreSingleSentences=1):
    '''
    ...
    '''
    assert tagger is not None, "must use tagger!"
...
#    data = data.ix[data[textType].dropna().index] #Drop nan rows
    ...
    lang = pd.DataFrame(index=[0],columns=columnList)
    for n in [0]:
        #keepAbstract = qc.identify_badabstracts(data[textType][n])
        if True: #This filters out any abstracts where the abstract is "no abstract" or includes "ABSTRACT TRUNCATED"
            if doPreprocessing==1:
                text = lang_prepropipeline(text,tagger)
            elif doPreprocessing==0:
                text = lang_minimalprepropipeline(text)
